...
bool CMysqlStore::ExecBindSql(const char *strMysql, const std::vector<string> &args)
{
	// 清除缓冲
	Clear();

	// 使用预处理语句插入二进制数据（推荐使用预处理语句来避免SQL注入）
	MYSQL_STMT *stmt;
	stmt = mysql_stmt_init(m_connptr);
	if (!stmt)
	{
		m_err = " mysql_stmt_init() failed";
		return false;
	}
	const char *query = strMysql;
	if (mysql_stmt_prepare(stmt, query, strlen(query)))
	{
		m_err = string(" mysql_stmt_prepare() failed:") + mysql_stmt_error(stmt);
		mysql_stmt_close(stmt);
		return false;
	}
	MYSQL_BIND *bind = new MYSQL_BIND[args.size()];
	memset(bind, 0, sizeof(MYSQL_BIND) * args.size());
	for (int i = 0; i < args.size(); ++i)
	{
		bind[i].buffer_type = MYSQL_TYPE_VAR_STRING; // 对于VARBINARY，使用 MYSQL_TYPE_VAR_STRING 或 MYSQL_TYPE_BLOB 都可以，但通常使用 MYSQL_TYPE_BLOB 以明确表示是二进制数据。
		bind[i].buffer = (void *)(args[i].c_str());
		bind[i].buffer_length = args[i].length(); // 注意，这里应该是数据的实际长度，不包括最后的null终止符（如果有的话）对于二进制数据通常不需要null终止符。
	}

	if (mysql_stmt_bind_param(stmt, bind))
	{
		m_err = string(" mysql_stmt_bind_param() failed:") + mysql_stmt_error(stmt);
		delete[] bind;
		mysql_stmt_close(stmt);
		return false;
	}
	...
	{
		m_err = string(" mysql_stmt_execute() failed:") + mysql_stmt_error(stmt);
		delete[] bind;
		mysql_stmt_close(stmt);
		return false;
	}
	m_row = (unsigned long)mysql_affected_rows(m_connptr);
	m_increaseID = (unsigned long long)mysql_insert_id(m_connptr);
	delete[] bind;
	mysql_stmt_close(stmt);
	return true;
}
...
void CMysqlStore::SetIncreaseID(unsigned long long id)
{
	m_increaseID = id;
}

//得到新自增的序列号
unsigned long long CMysqlStore::GetIncreaseID()
{
	return m_increaseID;
}
...
long long CMysqlStore::GetItemLongLong(unsigned long row,
	const unsigned int index)
{
	return atoll(GetItemValue(row, index).c_str());
}

long long CMysqlStore::GetItemLongLong(unsigned long row,
	const std::string &fieldname)
{
	return atoll(GetItemValue(row, fieldname).c_str());
}
...
bool CMysqlStore::Query(const std::string &dml)
{
	...
	do
	{
		...
		//取得各字段名和类型
		while((fieldptr = mysql_fetch_field(m_resultptr)) && (fieldptr!=NULL))
		{
			typeset_t typeset;
